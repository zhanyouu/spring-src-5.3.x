...
To retrieve a list of `entity` objects in Java, you would normally write the following:

[source,java,indent=0]
----
	Flux<entity> users  = client.get().retrieve().bodyToFlux(entity.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<entity>()
	// or (both are equivalent)
	val users : Flux<entity> = client.get().retrieve().bodyToFlux()
----
...
class UserHandler(builder: WebClient.Builder) {
	...
	suspend fun listView(request: ServerRequest): ServerResponse =
			...
			client.get().uri("...").awaitExchange().awaitBody<entity>()))
...
* `equals()` and `hashCode()`
* `toString()` of the form `"entity(name=John, age=42)"`
